...
# 가중치가 자동으로 재분배된다. 억지로 채우지 말 것.
#
# BCS 점수에 들어가는 지표는 전부 자동 계산된다. 여기 손으로 적는 것은
# 거시 축(LRS)의 수동 3종(DXY·연준·ETF)과 바닥선 두 개뿐이다.
# (LRS 의 'M2 임펄스'는 중국 M2 에서 자동 계산된다 — data/macro.csv, tools/fetch_macro.py.)

as_of: 2026-07-28        # 차트를 확인한 날짜

macro:
  # M2 임펄스는 data/macro.csv(중국 M2)에서 **자동 계산**된다 — 보통 비워 둔다.
  # 굳이 손으로 덮어쓰려면 값의 단위는 '중국 M2 전년비 − 24개월평균(%p, 대략 −3~+3)'
  # 이다. 글로벌 M2 전년비 '수준'(예: 5.2)을 적으면 자동값을 지우고 최대 완화로 오인된다.
  # m2_impulse:
